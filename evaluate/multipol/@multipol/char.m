function s = char(mp,xyzw,prec,printforstorage,numofactualvars)
% MULTIPOL/CHAR
    ...
    function c = term2str(p,t)
        ...
        for j=1:numel(jj)
            ...
            if xyzw && n<=4
                ...
                if printforstorage
                    if jj(j) <= numofactualvars
                        name = sprintf('a%u',jj(j));
                    else
                        name = sprintf('c%u',jj(j)-numofactualvars);
                    end
                else
